...
REQUIRES_BUILDER = ["ReplyParameters", "InlineKeyboardButton"]
...
def retrieve_info(url: str) -> dict:
    ...
    for x in list(dev_rules.children):  # type: Tag
        ...
        add_builder(curr_name, curr_type, items)
...
def add_builder(curr_name, curr_type, items):
    if curr_name in REQUIRES_BUILDER and curr_type is TYPES:
        items[curr_type][curr_name]["requires_builder"] = True
    ...
    for type_name, values in items[TYPES].items():
        ...
        for param in fields:
            ...
            for field_type_name in field_types:
                ...
                # Check if the field type ends with "[]"
                while field_type_name.endswith("[]"):
                    # Remove "[]" from the field type name
                    field_type_name = field_type_name[:-2]

                if field_type_name not in items[TYPES] and field_type_name not in TG_CORE_TYPES:
                    #if field_type_name[:-2] not in items[TYPES] and field_type_name[:-2] not in TG_CORE_TYPES:
